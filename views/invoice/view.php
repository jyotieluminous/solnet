...
<div style="max-width:800px;margin:auto;padding:30px;font-size:16px;line-height:24px;font-family:'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif;color:#555; ">
  ...
  <table cellpadding="7" cellspacing="0" style="line-height: inherit;text-align: left;width: 100%;">
          ...
          <tr>
            ...
            <td style="text-align:right;padding:30px 0px;">
				...
				<b style="font-size: 13px;">Printed Date: </b><?php echo date('d-M-Y',strtotime($model->invoice_date));  ?><br/>
           		...
           		<b style="font-size: 13px;">PO/WO/Contract no :</b><?php if(isset($model->customer->po_wo_number) && !empty($model->customer->po_wo_number)) { echo $model->customer->po_wo_number; } else { echo '-'; } ?>
            </td>
          </tr>
        </table></td>
    </tr>
    ...
    if(!empty($model->installation_fee)) { ?>
    <tr class="item">
    ...
    <?php } 
		$otf = 0;
		$otherFee = 0;
		if(!empty($model->other_service_fee)){
			$otherFee = $model->other_service_fee;
			$otf = 1;
		}elseif(!empty($model->linkcustomepackage->other_service_fee) && $otf == 0){
			$otherFee = $model->linkcustomepackage->other_service_fee;
		}
	
	
	
	?>
   <?php if(!empty($model->other_service_fee) || !empty($model->linkcustomepackage->other_service_fee)) { ?>
    <tr class="item">
      <td style="border-bottom: 1px solid #eee;"> Other Fee </td>
      <td style="text-align:right; border-bottom: 1px solid #eee;"><?php echo $model->linkcustomepackage->currency->currency." ".number_format($otherFee,2); ?> </td>
    </tr>
