...
class CustomerpackageController extends Controller
{
   ...
   /* public function behaviors()
    ...
    {        
        /* EQUIPEMENT TASK */
        $arrModel          = $this->findModel($id);
        $equpmentModel     = CustomerEquipments::find()->where(['fk_customer_id'=>$arrModel->fk_customer_id])->one();

        $arrResultEqupment = CustomerEquipments::find()->joinWith(['equmentData','equmentMacData'])->where(['fk_customer_id'=>$arrModel->fk_customer_id])->all();

        return $this->render('disconnect_view', [
            'model'             => $arrModel,
            'arrResultEqupment' => $arrResultEqupment,
            'equpmentModel'     => $equpmentModel,
        ]);
        /* END EQUIPEMENT TASK */
    }
    ...
    {
        $equpmentModel     = CustomerEquipments::find()->where(['id'=>$id])->one();

        $arrResultEqupment = CustomerEquipments::find()->joinWith(['equmentData','equmentMacData'])->where(['id'=>$id])->all();
        ...
        if(Yii::$app->request->post())
        {
            ...
            $strEquipmentType = $arrPostData['CustomerEquipments'];

            $strQuntity = $strEquipmentType['quantity'];

            //echo '<pre>';print_r($arrPostData['CustomerEquipments']['return_equipment_quntity']);echo '</pre>';

            $intEquipment = $arrPostData['CustomerEquipments']['return_equipment_quntity'] + $arrPostData['CustomerEquipments']['broken_equipment_quntity'];
           
            if($intEquipment < $strQuntity){
                $strStatus = 'partial';
            }elseif ($intEquipment == $strQuntity) {
                $strStatus = 'returned';
            }else{
                $strStatus = 'returned';
            }
            $customerEquipmentsModel                            = CustomerEquipments::findOne($id);
            $customerEquipmentsModel->return_equipment_quntity  = $arrPostData['CustomerEquipments']['return_equipment_quntity'];
            $customerEquipmentsModel->broken_equipment_quntity  = $arrPostData['CustomerEquipments']['broken_equipment_quntity'];
            if(isset($arrPostData['CustomerEquipments']['remark'])){
                 $customerEquipmentsModel->remark               = $arrPostData['CustomerEquipments']['remark'];
            }
            $customerEquipmentsModel->return_status             = $strStatus;
            $customerEquipmentsModel->save(false);
            
            $session->setFlash('success_msg',"Equipments Status Updated Successfully");
            return $this->redirect(Yii::$app->request->referrer);
        }

        return $this->renderAjax('update_status', [
            'id'                => $id,
            'arrResultEqupment' => $arrResultEqupment,
        ]);
    }
    
    public function actionUpdatemacstatus($id)
    {
        $arrResultEqupment = EquipmentsMacs::find()->where(['equipments_mac_id'=>$id])->all();
        ...
        if(Yii::$app->request->post())
        {
            ...
            $command = $connection->createCommand('UPDATE fin_equipments SET quantity_out = quantity_out - 1, quantity_in_hand = quantity_in_hand + 1 WHERE equipment_id ='.$macEquipmentsModel->fk_equipment_id);
            $command->execute();
        ...
        return $this->renderAjax('update_mac_status', [
            'id'                => $id,
            'arrResultEqupment' => $arrResultEqupment,
        ]);
    }
    ...
    */
    ...
    public function actionEquipmentreport()
    {
        ...
        return $this->render('return_goods_report', [
            'searchModel'  => $searchModel,
            'dataProvider' => $dataProvider,
            'user'         => $arrSalesPerson
        ]);
