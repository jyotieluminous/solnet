       ...
       /*  if(date('Y-m-d') != date('Y-m-01')){
        ...
        if(!empty($qryGetCustomer))
        {
            ...
            foreach($qryGetCustomer as  $key=>$val)
            {
                ...
                //$strInvoiceDate  = date('Y-m-d h:i:s');    // Shruti changed this
                $strInvoiceDate  = date('Y-m-1 h:i:s');
                ...
                //if(date('Y-m-d',strtotime($val->linkcustomerpackage->invoice_start_date))==date('Y-m-d'))  // Shruti changed this
                if(date('Y-m-d',strtotime($val->linkcustomerpackage->invoice_start_date))==date('Y-m-1'))
                {
                
                    //$qryGetCustInvc = Customerinvoice::find()->where(['fk_customer_id'=>$val->customer_id,'next_invoice_date'=>date('Y-m-d')])->orderBy('customer_invoice_id DESC')->one(); // Shruti changed this
                    $qryGetCustInvc = Customerinvoice::find()->where(['fk_customer_id'=>$val->customer_id,'next_invoice_date'=>date('Y-m-1')])->orderBy('customer_invoice_id DESC')->one();
                    if(!empty($qryGetCustInvc))   // If customer has any previous invoice
                    {
                        if($val->linkcustomerpackage->payment_type=='term')
                        {
                            ...
                            $newInvoiceModel->usage_period_from     = date('Y-m-1');        //Shruti changed this
                        ...
                        if($val->linkcustomerpackage->payment_type=='term')
                        {
                            ...
                            $newInvoiceModel->invoice_type          = 'normal';
                            $newInvoiceModel->invoice_date          = $strInvoiceDate;
                            //$newInvoiceModel->usage_period_from     = date('Y-m-d');      // Shruti Changed this
                            $newInvoiceModel->usage_period_from     = date('Y-m-1');
                            $newInvoiceModel->usage_period_to       = date('Y-m-t');
                            $newInvoiceModel->due_date              = $strDueDate;
                            $newInvoiceModel->fk_cust_pckg_id       = $strFkCustPckgId;
                            $newInvoiceModel->current_invoice_amount= $strPackagePrice;
