   ...
   */
    ...
    */
	...
	public function actionMonthlycron()
    {
        ...
        $qryGetCustomer = Customer::find()->joinWith('linkcustomerpackage')->where(['status'=>'active','is_invoice_activated'=>'yes','linkcustomepackage.payment_type'=>['term','advance'],'is_disconnected'=>'no'])->andWhere(['<=','linkcustomepackage.invoice_start_date',date("Y-m-01")])->andWhere(['invoice_generated' => '0'])->all();
        ...
        if(!empty($qryGetCustomer))
        {
            ...
            foreach($qryGetCustomer as  $key=>$val)
            {
                ...
                $strInvoiceDate  = date('Y-m-01 h:i:s');
                ...
                if(date('Y-m-d',strtotime($val->linkcustomerpackage->invoice_start_date))==date('Y-m-01'))
                {
                
                    $qryGetCustInvc = Customerinvoice::find()->where(['fk_customer_id'=>$val->customer_id,'next_invoice_date'=>date('Y-m-01')])->orderBy('customer_invoice_id DESC')->one();
                    if(!empty($qryGetCustInvc))   // If customer has any previous invoice
                    {
                        if($val->linkcustomerpackage->payment_type=='term')
                        {
                            ...
                            $strDueDate  =  date('Y-m-01',strtotime('+'.$intTermDays.'days'));
                            ...
                            $newInvoiceModel->usage_period_from     = date('Y-m-01');
                ...
                else
                {
                    ...
                    $qryGetCustInvc = Customerinvoice::find()->where(['fk_customer_id'=>$val->customer_id,'next_invoice_date'=>date('Y-m-01')])->orderBy('customer_invoice_id DESC')->one();
                        ...
                        if(!empty($qryGetCustInvc))  // CUSTOMER WITH NEXT INVOICE DATE AS TODAY's DATE
                        {
                            ...
                            $newInvoiceModel->created_at           = date('Y-m-d h:i:s');
                            $newInvoiceModel->next_invoice_date    = $strNextMonthFirstDate;
                            $strNextUsageDate =  date('Y-m-01',strtotime($lastdayNextUsageDateFrom.'+1 days'));
