...
class CustomercomplainController extends Controller
{
    ...
    */
    public function actionSubmitbrespond($id)
    {
        ...
        if (Yii::$app->request->isPost) {
            $arrPostData = Yii::$app->request->post(); 
            $arrPostData['Tblcustomercomplains']['job_completed_date']=  $arrPostData['Tblcustomercomplains']['support_date_time'];            
    ...
    */
    ...
    { 
        ...
        ## DATE RANGE FILTER 
        $checkBox = Yii::$app->request->post('selection');
        $usersID = '';
        ...
        $dataProvider = $searchModel->reportsearch($queryParams,$usersID);
        $dataProvider->pagination->pageSize=20;
     ...
     public function actionComplainreportexportcsv()
    { 
        $complainType = $name = $complain_type = $package_title = $installation_address = $issue = $actual_problem = $complain_date = $job_completed_date = $custid = '';
        $searchModel = new TblcustomercomplainsSearch();
        $searchModel->is_deleted= '0';
        ...
        { 
            $issue =  $_POST['issue'];}
        if(isset($_POST['actual_problem']))
        { 
            $actual_problem =  $_POST['actual_problem'];
        }
        if(isset($_POST['complain_date']))
        { 
            $complain_date =  $_POST['complain_date'];
        }
        if(isset($_POST['job_completed_date']))
        { 
        ...
        $dataProvider = $searchModel->reportsearch2($queryParams,$usersID,$name,$complain_type,$package_title,$installation_address,$issue,$actual_problem,$complain_date,$job_completed_date,$custid);
        $dataProvider->pagination->pageSize=1000;
        $filename = 'complainreport_'.date('Ymdhis').'.csv';
        ...
        if(!empty($dataProvider->getModels())) 
        {
            ...
            fputcsv($file, array("MTTR :".number_format($intMTTRHrs,2)." hours","Angka Gangguan / total dwon time :".$intTotalHours." hours","Penanganan Dalam 1 hari   / Case handle in 1 day :".number_format($intTotalCaseDays,2)." case a day"));
            fputcsv($file, array("Customer ID","Customer Name","State","Complain Type","Package Title","Package Speed","Installation Address","Problem","Actual Problem","Permanent solution","Date and Time Received Complain","Date and Time Job Completed","Total Down time (min)",));
                $data = array();
                foreach($dataProvider->getModels() as $key=>$value){
                    $to_time   = strtotime($value->complain_date);
                    $from_time = strtotime($value->job_completed_date); 
                    $intMinutes = round(abs($to_time - $from_time) / 60,2)." minute";
                    $installationAddress = trim(str_replace(', ',"-", $value->customer->linkcustomerpackage->installation_address));
                    $installationAddress = str_replace("\r\n", "-", $installationAddress);
                    $issue = str_replace("\r\n", "-", $value->issue);

                  $data[] = [
                        $value->customer->solnet_customer_id,
                        $value->customer->name,
                        $value->customer->state->state,
                        $value->complain_type,
                        $value->customer->linkcustomerpackage->package->package_title,
                        $value->customer->linkcustomerpackage->package_speed.' '.$value->customer->linkcustomerpackage->speed->speed_type,
                        $installationAddress,
                        $issue,
                        $value->actual_problem,
                        $value->permanent_solution,
                        date_format(date_create($value->complain_date),'d-m-Y  h:i A'),
                        date_format(date_create($value->job_completed_date),'d-m-Y h:i A'),
                        $intMinutes,
                    ] ;
                }
    ...
    public function actionComplainreportdownloadcsv(){
