...
public static function logActivity($logArray = array()) {
	...
	/* Function to generate invoice number backup*/ 
	 /*public function GetInvoiceId($lastInvoiceId = 0) {
		...
		public function GetCoplainTicketNo($stateId, $complainType) {		
			$selectArr = ['state_prefix','broadband_increment_id'];
			if($complainType == 'Dedicated')
				$selectArr = ['state_prefix','dedicated_increment_id'];
			else if($complainType == 'Local Loop')
				$selectArr = ['state_prefix','localloop_increment_id'];
						
			$getStatePrefix = State::find()->select($selectArr)->where(['state_id'=>$stateId])->one();
			
			if(!empty($getStatePrefix))
			{
				if($complainType == 'Broadband')
					$incID = $getStatePrefix->broadband_increment_id;
				else if($complainType == 'Dedicated')
					$incID = $getStatePrefix->dedicated_increment_id;
				else if($complainType == 'Local Loop')
					$incID = $getStatePrefix->localloop_increment_id;	
			...
			if ($incID > 0) {
			$lastInvoice = Tblcustomercomplains::find()->joinWith(['customer'])->where(['fk_state_id'=>$stateId,'complain_type'=>$complainType])->orderBy('created_at DESC')->one();
			...
			$incIdFormat = str_pad($incID, 7, '0', STR_PAD_LEFT);
			$ticketId = $incIdFormat."/".$prefix."/".$currentMonth."/".$currentYear;			
			$returnArray = array(); 
			$returnArray['increment_value'] = $incID;
			$returnArray['current_ticket_no'] = $ticketId;
	...
	*/
	/*public function updateCustomerIncrementValue($incID) {
	 ...
	 public function updateComplainIncrementValue($stateId,$incID,$complainType) {
		  
		  $sql = '';
		  $connection = yii::$app->db;
		  if($complainType == 'Broadband')
		  	$sql   = 'UPDATE tblstate SET broadband_increment_id = '.$incID.' where state_id = '.$stateId;
		  else if($complainType == 'Dedicated')
		  	$sql   = 'UPDATE tblstate SET dedicated_increment_id = '.$incID.' where state_id = '.$stateId;
		  else if($complainType == 'Local Loop')
		  	$sql   = 'UPDATE tblstate SET localloop_increment_id = '.$incID.' where state_id = '.$stateId;
		  if($sql != ''){
		  	$command = $connection->createCommand($sql);
		  	$result= $command->execute();
		  }		  
