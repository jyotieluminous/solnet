...
public static function logActivity($logArray = array()) {
	...
	*/
	...
	/*
	*  function to update the increament value for invoice (invoice number)
	*/
	...
	/*
	*  function to update the increament value for customer ID (solnet customre ID)
	*/
				...
				elseif($strPaymenType=='advance')
				{
					...
					if(strtotime($strLastDayOfMonth)>strtotime($strInvoiceStartDate))
					{
						...
						if($modelCustomerInvoice->save())
						{
								...
								$newModelCustomerInvoice->due_date  = 	date('Y-m-d', strtotime($strNextMonthFirstDate. ' + 3 days'));		// due_date should be last + 4 day of current month
								$arrInvoice = Yii::$app->customcomponents->GetInvoiceId();	
								if(!empty($arrInvoice))
								{
